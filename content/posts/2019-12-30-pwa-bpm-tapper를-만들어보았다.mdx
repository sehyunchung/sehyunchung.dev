...
> **웹앱** [`https://bipi.vercel.app`](https://bipi.vercel.app)  
> **깃헙** [`https://github.com/sehyunchung/bipi`](https://github.com/sehyunchung/bipi)

간단한 앱이다. '홈 화면에 추가'등을 이용해 디바이스에 설치할 수 있고, 화면 아무곳을 2번 이상 탭하면 BPM을 계산해서 보여준다. 스와이프를 하거나 4초간 입력이 없으면 0으로 리셋.
...
| 프로퍼티     | 설정시 화면 동작                                            | iOS                                          | fallback     |
| ------------ | ----------------------------------------------------------- | -------------------------------------------- | ------------ |
| `fullscreen` | 스탠드얼론 +                                                | 스탠드얼론으로 열림                          | `standalone` |
| `standalone` | 스탠드얼론                                                  | 스탠드얼론으로 열림                          | `minimal-ui` |
| `minimal-ui` | 스탠드얼론 앱으로 열리는데 메뉴바에 약간의 ui 컨트롤이 있음 | ..이라고 되어있는데 디폴트 브라우저에서 열림 | `browser`    |
