...
export const WORK_EXPERIENCE = [
	{
		...
		findings: [
			"FE 엔지니어의 업무 scope에 대해 보다 넓은 관점을 가지게 되었고",
			"기획 디자인 단계에서도 기여할 부분들을 찾게 되었음",
			"팀 빌딩에 관심과 재능이 있다는 사실을 발견",
			"감정적으로 포용적인 환경의 중요성 및 그를 위한 센스가 있다는 것을 알게 됨",
			"덩어리가 너무 큰 기획의 단점이 어떤 것인지 뼛속 깊이 깨닫게 됨",
			"그것을 쪼개기 위해 어떻게 커뮤니케이션 할 것인지",
			"유저의 직접 피드백의 중요성을 양보하지 않아야 한다는 생각을 하게 됨",
			"좀 더 빨리 깨닫고 보다 강력하게 주장해야 했었나 하는 아쉬움은 남아있음",
		],
	},
	{
		company: "(주) 버드뷰",
		...
		summary:
			"인턴으로 시작하여 정직원으로 채용되었습니다. 광고, 마케팅, 커머스 등 사업팀이 사용하는 어드민, 화해 앱 내 쇼핑 기능 관련 웹뷰 영역 등의 개발에 참여하며 프론트엔드 개발자로서 기본적인 능력을 갖추는 과정이었습니다.",
		findings: [
			"UI 개발이 무엇이고 어떤 것을 잘해야 하는 지 어느 정도 알게 된 경험",
			"성급한 추상화를 많이 저질렀고",
			"너무 많은 걸 알고 있는 컴포넌트를 많이 만들었고",
			"그로 인해 스스로 불러온 재앙을 많이 경험하여",
			"이거 이상한데...? 라는 생각을 하게 되었음",
			"태스크를 처리와 코드 너머의 시각은 아직 갖지 못한 상태",
		],
	},
]
...
export const MONO_PROJECTS = [
	{
		...
		whatIDid:
			"최초 외주 개발 홈페이지를 Gatsby.js 마이그레이션하는 작업부터 참여하여 각종 기능을 추가/개선하고 있습니다. 프레임워크 선정부터 배포 환경 설정까지 각종 기술적 결정을 내리고 변경에 유연한 컴포넌트/코드 구조를 리서치하고 제안하여, 팀의 개발 환경을 개선하는 데 기여하였습니다.",
		techStack:
	...
	},
	{
		...
		whatIDid:
			"기획/디자인 문서 자료가 없는 상태에서 전체 재작성 과정을 리딩했습니다. 기존 코드를 분석하고 히스토리를 아는 동료들과 인터뷰 및 시연 등을 진행하여 요구사항을 문서화하고, 안정화 기간을 지정해 테스트 항목을 보강하는 등 관련 기록을 정리해 기존의 문서 부재로 인한 비효율을 최소화하였습니다.",
		techStack:
	...
	},
	{
		...
		whatIDid:
			"계열사의 인력 부족으로 인해 파견된 프로젝트로, 단독으로 진행 및 완료했습니다. Next.js, TailwindCSS 및 배포 플랫폼으로 Vercel을 도입하는 테스트베드로 활용하여 이후 전체 프로젝트에 적용하였습니다.",
		techStack:
...
export const BIRDVIEW_PROJECTS = [
	{
		...
		whatIDid:
			"기존 PHP 기반 프로젝트에서 디자인을 리뉴얼하며 Next.js/Typescript로 재작성하는 프로젝트로, 비슷한 시기에 입사하신 시니어 개발자의 리드로 팀 전체가 React와 TypeScript를 학습하며 진행했습니다. 작은 단위의 컴포넌트를 개발하는 것부터 시작해 전체 프로젝트를 완성하는 경험을 할 수 있었습니다.",
	...
	},
	{
		...
		whatIDid:
			"서버 개발자들이 풀스택으로 개발 중이던 프로젝트를 프론트엔드 팀이 생기며 이어받아 개발하게 되었습니다. 기본적인 요구사항을 처리하는 것부터 시작해 복잡한 feature 단위의 태스크를 처리하는 능력을 갖추게 되었습니다.",
